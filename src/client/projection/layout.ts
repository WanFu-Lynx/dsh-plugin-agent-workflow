 ...
 */
/* jscpd:ignore-start -- The distributable Workflow plugin must own this fold instead of importing Trajectory's private implementation; see the visual-workflow Agent Note. */
import type {
  AssistantBlock,
...
type AssistantRequestView = Extract<WorkflowRequestView, { purpose: 'assistant' }>
 ...
 */
export function deriveWorkflowLayout(input: WorkflowLayoutInput): readonly WorkflowProjectionTurnModel[] {
  const {
  ...
  for (const entry of entries) {
    ...
    if (node.kind === 'compaction') {
      ...
      // duplicate workflow cell, but still advances the duration cursor.
      prevAbsTime = finiteTime(node.time) ?? prevAbsTime
      continue
    }
...
export function appendWorkflowPartialLayout(
  ...
  const partialTurn = deriveWorkflowLayout({
    nodes: [],
    partial,
    runningCalls: [],
  }).at(0)
  if (partialTurn === undefined) return turns
  const streamed: WorkflowProjectionTurnModel = {
    ...partialTurn,
...
function toTurnModel(
  turn: number | null,
  entry: TurnBucket,
): WorkflowProjectionTurnModel {
  const groups = entry.groups.map(({ title, laid }): WorkflowProjectionGroupModel => {
...
function firstCellIndex(turn: WorkflowProjectionTurnModel): number {
  return Math.min(
...
function expandAssistant(
  ...
  const thinkingText = node.blocks
    ...
    .join('\n\n')
  const message: WorkflowCellProps = {
    index: ++index,
...
function assistantSourceBlock(block: AssistantBlock): WorkflowSourceBlock {
  switch (block.kind) {
...
function sourceBlock(value: unknown): WorkflowSourceBlock {
...
function attachUsage(cell: WorkflowCellProps, usage: UsageLike | undefined): void {
...
function summarizeCall(
  name: string,
  argsRaw: string,
): Pick<WorkflowCellProps, 'text' | 'previewMarkdown'> {
  return {
    text: name,
...
function summarizeResult(
  node: ToolResultNode,
): Pick<WorkflowCellProps, 'result' | 'resultPreviewMarkdown'> {
  if (node.isError) {
...
function resultAsText(
  result: Pick<WorkflowCellProps, 'result' | 'resultPreviewMarkdown'> | undefined,
): Pick<WorkflowCellProps, 'text' | 'previewMarkdown'> {
  return {
    text: result?.result ?? '',
...
function previewContentProperty(
  content: readonly { type: string; text?: string }[],
): Pick<WorkflowCellProps, 'previewMarkdown'> {
