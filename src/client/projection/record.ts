/** Shared workflow record data and formatting contracts. */
...
/** Data and optional presentation attributes for one workflow record. */
export interface WorkflowCellProps extends HTMLAttributes<HTMLDivElement> {
  /** 1-based record index shown as `#N`. */
  index: number
  /** Projection-stable identity when no single source event owns the record lifecycle. */
  recordId?: string
  kind: WorkflowCellKind
  ...
  sourceBlocks?: readonly WorkflowSourceBlock[]
  /** Original tool result blocks in source order for the details panel. */
  outputBlocks?: readonly WorkflowSourceBlock[]
  ...
  /** Message-only prompt tokens not served from or written to a provider cache. */
  input?: number
 ...
 */
export function workflowRecordId(cell: WorkflowCellProps): string {
